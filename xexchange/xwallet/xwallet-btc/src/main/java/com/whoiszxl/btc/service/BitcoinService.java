...
import com.whoiszxl.core.entity.base.RpcParams;
import com.whoiszxl.core.utils.FastJsonUtils;
import com.whoiszxl.xwallet.core.entity.btc.ListTransactionResponse;
import com.whoiszxl.xwallet.core.entity.btc.RpcBaseResponse;
import com.whoiszxl.xwallet.core.entity.btc.TransactionInfoResponse;
...
public class BitcoinService {
    ...
    public String getBlockHeight() {
        Map params = RpcParams.builder().method("getblockcount").params(new Object[]{}).build().get();
        String blockHeight = bitcoinClient.getRequest(params);
        RpcBaseResponse rpcBaseResponse = FastJsonUtils.from(blockHeight, RpcBaseResponse.class);
        if(rpcBaseResponse.getError() != null) {
            log.warn("获取区块高度失败，节点报错信息为：{}", rpcBaseResponse.getError());
            return null;
        }
     ...
     * @return 地址对象
     */
    public String getNewAddress(String memberId) {
        Map params = RpcParams.builder().method("getnewaddress").params(new Object[]{memberId}).build().get();
        ...
        if(rpcBaseResponse.getError() != null) {
            log.warn("创建钱包地址失败，用户ID为：{}，节点报错信息为：{}", memberId, rpcBaseResponse.getError());
            return null;
        }
        return rpcBaseResponse.getResult();
    }
