...
import com.whoiszxl.xwallet.core.entity.Account;
...
public class EthService {
    ...
    public Result transferFromWithdrawWallet(String toAddress, BigDecimal amount, boolean sync, String withdrawId) {
    ...
    public Result transfer(String walletFile, String password, String toAddress, BigDecimal amount,boolean sync,String withdrawId) {
        Credentials credentials;
        try {
            credentials = WalletUtils.loadCredentials(password, walletFile);
        } catch (IOException e) {
            e.printStackTrace();
            return new Result(500, "钱包文件不存在");
        } catch (CipherException e) {
            e.printStackTrace();
            return new Result(500, "解密失败，密码不正确");
        }
        if(sync) {
    ...
    public Result transferFromWallet(String address, BigDecimal amount, BigDecimal fee, BigDecimal minAmount) {
        log.info("transferFromWallet 方法");
        List<Account> accounts = accountService.findByBalance(minAmount);
        if (accounts == null || accounts.size() == 0) {
            Result messageResult = new Result(500, "没有满足条件的转账账户(大于0.1)!");
        ...
        for (Account account : accounts) {
            ...
            Result result = transfer(coin.getKeystorePath() + "/" + account.getWalletFile(), "", address, realAmount, true,"");
    ...
    public Result transferToken(String fromAddress, String toAddress, BigDecimal amount, boolean sync) {
    ...
    public Result transferTokenFromWithdrawWallet(String toAddress, BigDecimal amount, boolean sync,String withdrawId) {
        Credentials credentials;
        try {
            //解锁提币钱包
